...
With Kafka in the cluster (Chapter~25), every service the project has is
deployable. The remaining milestones each swap something hand-rolled
...
\section{Frontend and the single host}

Milestone~7 serves the React frontend from the cluster and claims the
\code{/} path on \code{ts.local} that Chapter~18's Ingress deliberately
left free. One host, one origin --- no CORS, cookies and OAuth
redirects behave --- and the two-router division of Chapter~4 is
complete: Traefik fans out static assets to the frontend and
\code{/api} to the gateway.
