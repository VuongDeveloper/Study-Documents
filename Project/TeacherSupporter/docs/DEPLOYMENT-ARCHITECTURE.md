...
```
   ...
   :443 ──────┼─┼─> [ Caddy ]  (TLS termination, only        │ │
              ...
              │ │      └─ /ts-submissions/* → [ alarik:8080 ] │ │
              │ │                          │                 │ │
              │ │   [ api-gateway ]────────┤ lb:// via       │ │
              │ │      │                   │ Eureka          │ │
              │ │      ├─> [ auth-service ]──> postgres-auth │ │
              │ │      ├─> [ course-service ]─> postgres-course, alarik
              ...
              │ │     alarik-data, caddy-data                │ │
              │ └────────────────────────────────────────────┘ │
              └────────────────────────────────────────────────┘
                                    ▲
   :22 ── SSH ──────────────────────┘
          └─ tunnels for internal UIs: Eureka :8761, Zipkin :9411,
             Kafka UI :9090, Alarik console :9001, MailDev :1080
```
...
2. **Everything else is network-internal.** Databases, Kafka, Eureka, config-server are reachable
   ...
   list.
3. **Operator access is SSH-only.** Dashboards (Eureka, Zipkin, Kafka UI, Alarik console, MailDev)
...
4. **State lives in named volumes.** Containers are disposable; `postgres-*-data`, `mongo-data`,
   `alarik-data` survive `compose down` and image upgrades. Backup story = backing up these
...
config-server ──healthy──> discovery-server ──> api-gateway
      ...
      ├──healthy──> course-service      (also waits: postgres-course, kafka, alarik)
...
| Secrets | defaults in compose (`root`, `tssecretkey123`) | server-side `.env`, chmod 600 |
