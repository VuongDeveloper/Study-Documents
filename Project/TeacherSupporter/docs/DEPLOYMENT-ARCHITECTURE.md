...
```
   ...
   :443 ──────┼─┼─> [ Caddy ]  (TLS termination, only        │ │
              ...
              │ │      └─ /ts-submissions/* → [ garage:3900 ]│ │
              │ │                          │                 │ │
              │ │   [ api-gateway ]────────┤ lb:// via       │ │
              │ │      │                   │ Eureka          │ │
              │ │      ├─> [ auth-service ]──> postgres-auth │ │
              │ │      ├─> [ course-service ]─> postgres-course, garage
              ...
              │ │     garage-data, caddy-data                │ │
              │ └────────────────────────────────────────────┘ │
              └────────────────────────────────────────────────┘
                                    ▲
   :22 ── SSH ──────────────────────┘
          └─ tunnels for internal UIs: Eureka :8761, Zipkin :9411,
             Kafka UI :9090, Garage admin API :9003, MailDev :1080
```
...
2. **Everything else is network-internal.** Databases, Kafka, Eureka, config-server are reachable
   ...
   list.
3. **Operator access is SSH-only.** Dashboards (Eureka, Zipkin, Kafka UI, Garage admin API, MailDev)
...
4. **State lives in named volumes.** Containers are disposable; `postgres-*-data`, `mongo-data`,
   `garage-data` survive `compose down` and image upgrades. Backup story = backing up these
...
config-server ──healthy──> discovery-server ──> api-gateway
      ...
      ├──healthy──> course-service      (also waits: postgres-course, kafka, garage healthy)
...
| Secrets | defaults in compose (`root`, the `GKts…dev0` Garage key) | server-side `.env`, chmod 600 |
