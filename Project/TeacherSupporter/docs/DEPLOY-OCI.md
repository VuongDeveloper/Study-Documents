...
3. **Real secrets** from a server-side `.env` (`chmod 600 .env`, never committed):
   `POSTGRES_PASSWORD`, RustFS credentials, `GOOGLE_CLIENT_ID/SECRET`, JWT signing key.
...
- **`APP_S3_PUBLIC_ENDPOINT`** currently `http://localhost:9000` — must become a public URL,
  because presigned URLs are opened by the *browser*. Route `https://<domain>/files/*` → `rustfs:9000`
...
- **MailDev** accepts and displays all mail with zero auth. Either keep it unpublished (view via
  ...
  password).
- **Kafka UI / Zipkin / RustFS console / Eureka dashboard**: never publish; use SSH tunnels (§5.4).
...
### 5.4 Reaching internal UIs (Eureka, Kafka UI, Zipkin, RustFS, MailDev)
